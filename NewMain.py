...
for i in range(0,np.shape(pt)[0]):
    point=[pt[i,0],pt[i,1]]
    points.append(point)
...
##此时results代表外围的点
outernum=[]
innernum = list(range(0,np.shape(pt)[0]))
...
np.savetxt(filename+'outer.csv',outerpoint,fmt='%f',delimiter=',')
np.savetxt(filename+'sortinner.csv',sortinner,fmt='%f',delimiter=',')
np.savetxt(filename+'sortouter.csv',sortouter,fmt='%f',delimiter=',')
