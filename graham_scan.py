...
def sort_polar_angle_cos(points, center_point,bindex):
    """
    ...
    for i in range(0, n - 1):
        ...
        while index > 0:
            ...
            if cos_value[index] > cos_value[index - 1] or (
                ...
                #交换norm_list[index]和norm_list[index-1]
    ...
    for i in rank:
        if i>= bindex:
            sorted_points.append([points[i-1],i])
        else :
            sorted_points.append([points[i],i])
...
def graham_scan(points):
    ...
    bottom_point = points.pop(bottom_index)#pop() 函数用于移除列表中的一个元素（默认最后一个元素），并且返回该元素的值。
    sorted_points = sort_polar_angle_cos(points, bottom_point,bottom_index)
    ...
    stack = []
    stack.append([bottom_point,bottom_index])
    stack.append(sorted_points[0])
    stack.append(sorted_points[1])##保证能够首尾相连

    for i in range(2, m):
        length = len(stack)
        top = stack[length - 1]
        next_top = stack[length - 2]
        v1 = [sorted_points[i][0][0] - next_top[0][0], sorted_points[i][0][1] - next_top[0][1]]
        v2 = [top[0][0] - next_top[0][0], top[0][1] - next_top[0][1]]

        while coss_multi(v1, v2) >= 0:##两个向量叉乘
            stack.pop()#删掉最后一个值
            length = len(stack)
            top = stack[length - 1]
            next_top = stack[length - 2]
